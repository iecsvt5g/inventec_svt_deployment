...
import time,datetime , socket,re
...
def schedule_task2() -> None:   # 5 sec schedule task
    tasks = ["service","alarm","system","ru","acc","bler","gnbid"]
    for t in tasks:
        print("./mybbu --{0}".format(t))
        MYBBU["{0}".format(t.upper())] = run_command("./mybbu --{0}".format(t))
...
async def update():
    ...
    tasks = ["service","alarm","system","ru","acc","bler","bmc","gnbid"]
    for t in tasks:
        print("./mybbu --{0}".format(t))
        MYBBU["{0}".format(t.upper())] = run_command("./mybbu --{0}".format(t))
...
# GET gnbid
@app.get("/gnbid", tags=["BBU"])
def gnbid():
    return MYBBU.get("GNBID")


from bbu_log_api import bbu_log

@app.get("/phy_log")
def phy_log():
    return bbu_log.phy_log()

@app.get("/du_log")
def du_log():
    return bbu_log.du_log()

@app.get("/cu_log")
def cu_log():
    return bbu_log.cu_log()
